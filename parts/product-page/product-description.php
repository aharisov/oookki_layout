<article class="product-description">
    <h2>Le nouveau Galaxy Z Fold6 est là !</h2>

    <div class="description-inner">
        <section class="description-section">
            <div class="text">
                <h3>Un Galaxy Z Fold plus léger et plus fin qui se glisse partout</h3>
                <p>Le Galaxy Z Fold6 se décline en un éventail de couleurs qui feront tourner les têtes, y compris de nouveaux coloris Blanc et Noir Carbone, disponibles en exclusivité sur samsung.com</p>
            </div>
            <div class="media">
                <figure class="common-video__figure loaded">
                    <video controls="" autoplay="" name="media">
                        <source src="https://images.samsung.com/fr/smartphones/galaxy-z-fold6/videos/galaxy-z-fold6-features-highlights-design.webm?imbypass=true" type="video/webm">
                    </video>
                </figure>
            </div>
        </section>
        <section class="description-section">
            <div class="text">
                <h3>Résumez vos idées en un cin d’oeil</h3>
                <p>Que vous preniez des notes en cours ou rédigiez votre mémoire, l'Assistant Notes résume et compile ce que vous écrivez en quelques mots.2 Pour tout le reste, le S Pen vous aide à griffonner vos inspirations sur grand écran.
                </p>
            </div>
            <div class="media">
                <figure>
                    <video controls="" autoplay="" name="media">
                        <source src="https://images.samsung.com/fr/smartphones/galaxy-z-fold6/videos/galaxy-z-fold6-features-highlights-note-assist.webm?imbypass=true" type="video/webm">
                    </video>
                </figure>
            </div>
        </section>
        <section class="description-section">
            <div class="text">
                <h3>Le processeur Snapdragon le plus rapide à ce jour</h3>
                <p>Le Galaxy Z Fold6 est doté d'un écran incroyablement lumineux. Grâce à la technologie Vision Booster, vous profitez de vos sessions de jeu, même en plein soleil.</p>
            </div>
            <div class="media">
                <figure>
                    <video controls="" autoplay="" name="media">
                        <source src="https://images.samsung.com/fr/smartphones/galaxy-z-fold6/videos/galaxy-z-fold6-features-highlights-performance-a.webm?imbypass=true" type="video/webm">
                        <source src="https://images.samsung.com/fr/smartphones/galaxy-z-fold6/videos/galaxy-z-fold6-features-highlights-performance-a.mp4?imbypass=true" type="video/mp4">
                    </video>
                </figure>
            </div>
        </section>
        <section class="description-section">
            <div class="text">
                <h3>Entourer pour chercher</h3>
                <p>Recherchez comme jamais auparavant avec Entourer pour chercher. A l'aide de votre doigt ou de votre S Pen, entourez simplement un élément sur l'immense écran du Galaxy Z Fold6 pour obtenir instantanément des résultats de recherche Google.</p>
            </div>
            <div class="media">
                <figure>
                    <video controls="" autoplay="" name="media">
                        <source src="https://images.samsung.com/fr/smartphones/galaxy-z-fold6/videos/galaxy-z-fold6-features-highlights-cts.webm?imbypass=true" type="video/webm">
                    </video>
                </figure>
            </div>
        </section>
    </div>
</article>